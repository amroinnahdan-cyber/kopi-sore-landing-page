...
export default function CartDrawer() {
  ...
  const message = `Halo Kopi Sore, saya ingin pesan:%0A%0A${items.map((item, index) => `${index + 1}. ${item.name} x${item.qty}%0A   ${item.temperature || "Iced"} · ${item.sweetness || "Normal"}%0A   ${rupiah(item.price * item.qty)}`).join("%0A%0A")}%0A%0ATotal: ${rupiah(totalPrice)}`;
